...
This project showcases the complete setup, prerequisites installation, and configuration process for deploying the osTicket Helpdesk System on a Microsoft Azure Virtual Machine (VM) via Remote Desktop Protocol (RDP)<br />


<h2>Environments and Technologies Used For Prerequisites</h2>
...
1. This project demonstrates the configuration of IIS on Windows 10, including enabling CGI support required for web-based applications such as osTicket. It also illustrates how IIS interacts with a web browser (e.g., Microsoft Edge) and how it can be used for local hosting, testing, and development environments.

2. Web Platform Installer — A Microsoft tool that simplifies the installation and management of web platform components, including IIS, PHP, and MySQL.
   
3. Install Microsoft Visual C++ Redistributable on Windows 10 — Required because PHP and MySQL components depend on these runtime libraries.


 
<img width="1793" height="928" alt="image" src="https://github.com/user-attachments/assets/86c616bb-66c2-4744-bcd6-1035a770468e" />

IIS (Internet Information Services) Configuration on Windows 10
The diagram above illustrates how IIS interacts with a web browser (Microsoft Edge) on Windows 10, followed by a step-by-step demonstration of the configuration process.
This setup includes enabling CGI support, which is required for applications like osTicket running under the World Wide Web Services (WWS) component in IIS.

1.Enabling IIS with CGI Support:
Steps:-Open the Windows Features dialog, -Press Windows + R → type optionalfeatures → press Enter, -Scroll to Internet Information Services (IIS) and check the box. -Expand Web Management Tools and World Wide Web Services. -Expand Application Development Features and enable CGI. -Click OK to install, then select Close.
2.Verifying IIS Installation:
-Open a web browser (Edge, Chrome, or Firefox), -Enter:http://localhost or system's IP address. Now the IIS Welcome Page, confirming successful activation.
Editing the Default IIS Web Page: After activation, IIS creates a default web directory where content can be served to the browser.
Steps:-Open Notepad as Administrator,-Go to File → Open,-Navigate to: C:\inetpub\wwwroot,-Change file type to All Files,-Open iisstart.htm,-Edit the file as needed,-Save changes
Refresh the browser — the updates will appear instantly.
The Windows 10 system is now ready to:-Host local websites and development environments (e.g., osTicket).-Run PHP and MySQL-based web applications.-Simulate real-world web hosting environments for training and development.
</p>
<br />
...
After IIS (Internet Information Services) is enabled, it automatically creates a default web directory that allows hosting and accessing files via a web browser. To view and modify the default IIS landing page, follow these steps:-Open Notepad and run it as Administrator.-Go to File → Open.-Navigate to: C:\inetpub\wwwroot,-In the file selector, change the filter to All Files to view all items in the directory.-Open the file named iisstart.htm — this is the default IIS web page source file.-Edit the file as needed, then click Save.
Once saved, refresh your browser. The IIS home page will update immediately, reflecting your changes (as shown in the reference diagram).
...
How to Install Microsoft Visual C++ Redistributable on Windows 10
Follow these steps to install the Microsoft Visual C++ Redistributable, which is required for applications like PHP, MySQL, and other software components that depend on the Visual C++ runtime libraries.
Step 1: Download the Redistributable:-Visit the official Microsoft download page: Go to the Microsoft Visual C++ Redistributable Downloads
 page.-Select the appropriate version: .For 64-bit systems, download the x64 version. .For 32-bit systems, download the x86 version.-Click the corresponding link to start the download. Step 2: Run the Installer:-Locate the downloaded file:
Navigate to your Downloads folder and find the installer (e.g., vc_redist.x64.exe).-Execute the installer:
Double-click the file to run it.-Accept the license agreement:
Read the terms, check the box to accept, and click Install.-Wait for installation to complete:
The process typically takes a few minutes. Once finished, click Close.
Step 3: Verify Installation:-Open Control Panel:
Press Windows + R, type appwiz.cpl, and press Enter.-Check installed programs:Look for entries such as: .Microsoft Visual C++ 2015–2022 Redistributable (x64) .Microsoft Visual C++ 2015–2022 Redistributable (x86) If these appear, the installation was successful. Step 4:Restart Your Computer (Optional): To ensure all components are properly initialized, it is recommended to restart your system. By following these steps, the Microsoft Visual C++ Redistributable will be installed on your Windows 10 machine, enabling applications that depend on it to run correctly.




<h2>Environments and Technologies Used For Configuration Process</h2>
...
<h2>Installation and Configuration Objectives</h2>
